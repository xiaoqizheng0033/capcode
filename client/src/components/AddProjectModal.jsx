...
export default function AddProjectModal({ open, onClose, onAdded }) {
    ...
    try {
      ...
      while (true) {
        ...
        while (buffer.includes('\n\n')) {
          ...
          // Skip SSE comments and non-events
          if (!eventType && !eventData) continue
          if (!eventType && eventData) eventType = 'message'
          ...
          try {
            ...
            if (eventType === 'progress') {
                ...
                for (const l of progressLines) {
                  ...
                  if (last && last.type === 'info' &&
                      (last.text.startsWith('Receiving') ||
                       ...
                       last.text.startsWith('remote:') ||
                       last.text.startsWith('Cloning'))) {
                    next[next.length - 1] = { type: 'info', text }
                  } else {
              ...
              setLogs(prev => [...prev, { type: 'success', text: 'Clone complete' }])
              setLoading(false)
              setUrl('')
  ...
  return (
    <div className="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50" onClick={loading ? handleCancel : onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          />
          {repoName && (
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Save to: C:\Myfiles\Codes\repos\{repoName}</p>
          )}

          {logs.length > 0 && (
            <div className="mt-3 bg-gray-950 text-green-400 rounded-md p-3 font-mono text-xs max-h-64 overflow-y-auto">
              {logs.map((log, i) => (
          ...
          <div className="flex justify-end gap-3 mt-4">
            <button type="button" onClick={handleCancel} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">Cancel</button>
            <button type="submit" disabled={loading || !url.trim()} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
